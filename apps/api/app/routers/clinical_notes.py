...
def _row_to_schema(row: ClinicalNoteModel) -> ClinicalNote:
    return ClinicalNote(
        ...
        patientTolerance=PatientTolerance(row.patient_tolerance) if row.patient_tolerance else None,
        complications=row.complications,
        treatmentRendered=row.treatment_rendered,
        nextVisitPlan=row.next_visit_plan,
        notes=row.notes,
        templateType=TemplateType(row.template_type) if row.template_type else None,
