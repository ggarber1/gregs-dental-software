"""
Unit tests for GET/POST/PATCH/DELETE /api/v1/appointment-types.
...
_APPOINTMENT_TYPE_ROW_DEFAULTS: dict[str, Any] = {
...
def _make_row(**overrides: Any) -> MagicMock:
    row = MagicMock()
    for k, v in {**_APPOINTMENT_TYPE_ROW_DEFAULTS, **overrides}.items():
        setattr(row, k, v)
    return row
...
def _copy_attrs(target: Any, source: MagicMock) -> None:
    ...
    for key in _APPOINTMENT_TYPE_ROW_DEFAULTS:
...
@pytest.mark.asyncio
async def test_create_appointment_type_returns_201():
    app = _get_app()
    mock_row = _make_row()

    with (
        _auth_patches() as auth_headers,
        patch("app.routers.appointment_types.get_session_factory") as mock_sf,
    ):
...
async def test_list_appointment_types_returns_list():
    app = _get_app()
    row1 = _make_row(name="Cleaning")
    row2 = _make_row(id=uuid.uuid4(), name="Exam")
    ...
    with (
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            response = await c.get("/api/v1/appointment-types", headers=auth_headers)

    assert response.status_code == 200
    body = response.json()
    assert isinstance(body, list)
    assert len(body) == 2
    assert body[0]["name"] == "Cleaning"
    assert body[1]["name"] == "Exam"
...
@pytest.mark.asyncio
async def test_get_appointment_type_returns_200():
    app = _get_app()
    mock_row = _make_row()
    ...
    with (
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            response = await c.get(
                f"/api/v1/appointment-types/{_APPOINTMENT_TYPE_ID}",
...
@pytest.mark.asyncio
async def test_update_appointment_type_returns_200():
    app = _get_app()
    mock_row = _make_row(name="Deep Cleaning", duration_minutes=60)
    ...
    with (
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            response = await c.patch(
                f"/api/v1/appointment-types/{_APPOINTMENT_TYPE_ID}",
                json={"name": "Deep Cleaning", "durationMinutes": 60},
                headers={**auth_headers, "Idempotency-Key": str(uuid.uuid4())},
            )
...
@pytest.mark.asyncio
async def test_delete_appointment_type_returns_204():
    app = _get_app()
    mock_row = _make_row()
...
@pytest.mark.asyncio
async def test_create_appointment_type_no_practice_scope_returns_403():
    app = _get_app()
...
@pytest.mark.asyncio
async def test_create_appointment_type_read_only_role_returns_403():
    app = _get_app()
