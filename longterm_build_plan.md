...
- 🔄 **Module 8 — Billing & Payments** — split into **8a** and **8b+**. **8a (patient ledger, built):** append-only `ledger_entries` table (migration 0034) + ledger service computing running balance and posting entries — auto-posted charges on appointment checkout, insurance payment + non-PR contractual write-off from ERA, record-only patient payments (Stripe card processing deferred), manual adjustments, and reversals; feature-gated `billing_ledger` router (GET ledger / POST payments / POST adjustments / POST reverse); patient-chart Ledger tab; all hooks (checkout + ERA poll) soft-gated on `billing_ledger`. Spec: `specs/2026-06-23-module-8a-patient-ledger-design.md`. **8b+ (🔲 remainder):** patient-facing statements (email/print), QuickBooks export, full guarantor/family accounts, and Stripe card processing. (requires 7)
  - **8b first slice — Insurance A/R Worklist (aging + underpayment) — ✅ BUILT, PR #61** (spec: `specs/2026-06-29-module-8b-insurance-ar-design.md`; plan: `plans/2026-06-29-module-8b-insurance-ar.md`). Read-mostly worklist over `claims` + the Module 6 estimate: **awaiting-carrier** (aged, chase oldest first), **underpaid** (auto-detected when insurance pays >5% below the Module 6 estimate → Accept / Flag-for-appeal), **problem** (denied/rejected/failed with Stedi reason), and a manual **appealing** triage state. Plus a birds-eye carrier × age-bucket summary. Classification keys off the numbers, not the (possibly-wrong) `paid`/`partially_paid` status label. One migration (`claims.insurance_reviewed_at`). **Underpayment detection folded in** (the high-value half of reconciliation); overpayment/refund reconciliation + patient-aging/statements remain follow-on siblings. Chosen as the first 8b build because it's low-risk/self-contained and *surfaces* the recovery work — [[Claim recovery & appeals]] (resubmission/real appeals/275) is deferred to after Staging Checkpoint 5, since it extends the still-unverified live submission path.

**Open checkout/payment workflow questions — discuss with dad (gates 8b UX):**

8a posts charges when an appointment is marked `completed` and records payments in the patient's Ledger tab. Before building the 8b checkout UX, confirm how the real front-desk flow works:

- **Pay-on-the-spot timing:** Patients sometimes pay immediately at checkout, so the **total owed must be computed and visible at the moment of completion** — not just the gross fee. Need patient-responsibility (gross − insurance estimate, from Module 6 co-pay) ready to collect at checkout. Confirm: do they always collect at checkout, or sometimes bill later?
...
| **Claim recovery & appeals (NOT built — gap surfaced during Module 8b design, 2026-06-29).** Today there is *no* recovery path for a claim that rejects, fails, or is denied: (a) **no resubmission** — `claims.submission_attempt` exists but is never incremented and there's no fix-and-resend endpoint (rejected/failed claims that never reached the carrier are stuck); (b) **no appeal workflow** — the `appealing` status is reserved but *no code ever sets it*; a real appeal = corrected-claim resubmission and/or appeal documentation (couples to **275 attachments** above); (c) **denied claims** get `status=denied` + stored CARC codes from 7b, then nothing — no review step and the gross charge sits on the patient's books. **Module 8b makes denied/rejected/failed *visible* and adds a triage flag only** (`appealing` as a marker + `insurance_reviewed_at`); it does **not** submit appeals or resubmit. **Also resolve the open discrepancy:** does a *denied* claim post a contractual write-off to the ledger? 8a memory says CO/OA auto-write-off; the 7b ERA-posting code only posts write-offs alongside an insurance *payment* — pin down the intended ledger treatment of a denial. | Module 8b design (2026-06-29) | When a real practice starts working denials/rejections — likely soon after first onboarding; the denial-write-off discrepancy should be resolved sooner |
| **Module 8b sibling reports (next up after PR #61).** (a) **Reconciliation worklist** — once an ERA posts, compare estimated insurance vs. actual `insurance_paid_cents` and surface the ledger adjustment (the dentist's checkout-then-reconcile step 2); reuses 8b's estimate-resolution logic. (b) **Patient aging + statements** — patient-grouped A/R from the Module 8a ledger → email/print statements (the "patient didn't pay at checkout, bill them" path). (c) **Overpayment/refund** handling. Above all three, a practice A/R dashboard (insurance owes $X / patients owe $Y). | Module 8b design (2026-06-29) | Reconciliation first (highest revenue value); patient aging/statements next |
| **Observability story (NOT built — flagged 2026-06-29).** No real monitoring/alerting yet. Highest priority: **Stedi health** — failure/error-rate spike alerts across all call types, a distinct **auth-failure** alert (dead/expired key), cost/volume anomaly (retry storms), and claim **rejection-rate** spikes. Plus revenue-cycle SLOs (claims aged past N days, ERA unmatched-queue growth, eligibility failure rate); app/infra basics (API 5xx + p50/p95/p99 latency, DB pool/slow-query/migration-failure, worker queue depth + dead-letter); and **HIPAA** PHI-access audit logging + auth-failure alerting. Open stack decision: CloudWatch alarms+logs vs Datadog/Sentry; SNS→email/Slack/PagerDuty sink; request/trace IDs threaded through Stedi calls. Deserves its own spec. | Greg, 2026-06-29 (Module 8b session); ties to `research/16_cost_and_scaling_model.md` + AWS BAA | Before onboarding the first real practice (HIPAA audit + Stedi alerting); revisit each Stedi-touching module |
